...
BEGIN
	...
	WHERE
		DATE(DATA_INICIO) = DATE(DIA) AND TEMP_TAREFAS.FK_USUARIO = USUARIO
	ORDER BY DATA_INICIO;
...
BEGIN
	...
	DECLARE oTarefas CURSOR FOR 
        ...
        WHERE TAREFA.FK_USUARIO = USUARIO AND (CONCLUIDO = FALSE OR DATA_CONCLUIDO = DATE(NOW())) ORDER BY IMPORTANCIA DESC;
...
BEGIN
    ...
    WHILE DATA_INICIAL <= DATA_FINAL DO
		SELECT COUNT(COD_TAREFA) INTO oQTD FROM TAREFA WHERE DATA_CONCLUIDO = DATA_INICIAL AND FK_PROJETO = COD_PROJETO;
...
BEGIN
    ...
    WHILE DATA_INICIAL <= DATA_FINAL DO
		SELECT COUNT(COD_TAREFA) INTO oQTD FROM TAREFA WHERE DATA_CONCLUIDO = DATA_INICIAL AND FK_PROJETO = COD_PROJETO AND FK_USUARIO = COD_USUARIO;
        INSERT INTO DATA_TAREFAS(QTD_TAREFAS, DATA_TAREFA) VALUES (oQtd, DATA_INICIAL);
        SET DATA_INICIAL =  DATE_ADD(DATA_INICIAL, INTERVAL 1 DAY);
    END WHILE;
