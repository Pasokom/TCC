...
public class IOFunctions {
	...
	private final String[] extensions = new String[] { "ser", "png", "jpg", "gif", "jpeg" };
	...
	public boolean doSerialization(Object obj, String fileName) {
		String folder = getSerializationFolder();
		File f = new File(folder);
		if (!f.exists())
			f.mkdirs();
		final String serialFolder = f.getPath() + "/" + fileName;
		File serial = new File(serialFolder);
		deleteFileIfExists(serial);
		try {
			FileOutputStream file = new FileOutputStream(getSerializationFolder() + fileName + ".ser");
	...
	public Object undoSerialization(String fileName) {
		try {
			FileInputStream fis = new FileInputStream(getSerializationFolder() + fileName + ".ser");
	...
	public boolean fileExists(String fileName) {
		final String path = getSerializationFolder() + fileName + ".ser";
...
//			System.out.println(file.getPath());
		...
		final String filePath =file.getPath().toString().substring(0,index) + "/";
...
//		System.out.println(f.getPath().toString());
	...
	public String getSerializationFolder() {
		final String path = getConfigFolder() + "serialization/";
