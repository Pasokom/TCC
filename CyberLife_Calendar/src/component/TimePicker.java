...
public class TimePicker extends HBox{
	...
	public TimePicker() {
		...
		hour.setOnMouseClicked(e -> { timeSelector.getChildren().set(1, horaPane()); });
		min.setOnMouseClicked(e -> { timeSelector.getChildren().set(1, minutoPane()); });
		...
		btnCancelar.setOnAction(e -> { timeSelectorStage.close(); });
		btnOK.setOnAction(e -> { 
	...
	private Pane minutoPane() {
		...
		for (int i = 0; i < 12; i++) {
			...
			circulos[i].setOnMouseClicked(e -> { 
				...
				this.min.setText(String.format("%02d", Integer.parseInt(((Label)circulos[grau / 30].getChildren().get(1)).getText()))); 
			});
